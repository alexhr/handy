...
jQuery(document).ready(function ($) {
	//Cache some variables
	var images = $('#slide-2 p a');

	images.hover(
		function(e) {
			var asta = $(this).find('img');
			$('#slide-2 img').not( asta ).stop(false, false).animate(
				{
					opacity: .5
				},
				'fast',
		...
		function(e) {
			$('#slide-2 img').stop(false, false).animate(
				{
					opacity: 1
				},
				'fast',
...
function GmapInit() {
	  ...
	  Gmap.each(function() {
		...
		if( dataContent !== undefined && dataContent !== false ) {
			contentString = '<div class="map-data">' + '<h5>' + title + '</h5>' + '<div class="map-content">' + dataContent + '</div>' + '</div>';
		}
